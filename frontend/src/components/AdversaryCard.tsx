    ...
    <div className="p-4 rounded-xl border border-purple-500/20 bg-purple-500/5 space-y-3">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <h3 className="text-base font-bold text-white">{adversary.name}</h3>
          <p className="text-xs text-[#64748B] mt-0.5">{adversary.motivation}</p>
        </div>
        <div className="text-right shrink-0">
          <p className="text-2xl font-bold font-mono text-purple-400">{adversary.confidence}%</p>
          <p className="text-[10px] text-[#334155] font-mono">CONFIDENCE</p>
        </div>
      </div>

      {adversary.ttps.length > 0 && (
        <div>
          <p className="text-[10px] text-[#334155] font-mono uppercase tracking-wider mb-1.5">Matched TTPs</p>
      ...
      <p className="text-xs text-[#475569] leading-relaxed border-t border-purple-500/10 pt-3">{adversary.notes}</p>
    </div>
  )
}
