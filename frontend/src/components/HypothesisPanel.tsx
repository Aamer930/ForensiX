...
const RANK_COLORS = [
  ...
  { bar: 'bg-[#475569]', text: 'text-[#64748B]', border: 'border-[#1E293B]', bg: 'bg-[#0F172A]' },
]
  ...
  if (!hypotheses.length) {
    return (
      <div className="p-4 rounded-lg border border-[#1E293B] bg-[#0F172A]">
        <p className="text-sm text-[#94A3B8] leading-relaxed">{primary}</p>
      </div>
    )
  }
      ...
      {hypotheses.map((h, i) => {
        ...
        return (
          <div key={i} className={`p-4 rounded-lg border ${c.border} ${c.bg}`}>
            ...
            <div className="h-1 bg-[#0F172A] rounded-full mb-3 overflow-hidden border border-[#1E293B]">
              <div
