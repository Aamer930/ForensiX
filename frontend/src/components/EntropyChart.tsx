    ...
    <div className="p-5 rounded-xl border border-[#1E293B]" style={{ background: 'rgba(15,23,42,0.6)' }}>
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-mono text-[#64748B] uppercase tracking-wider">File Entropy Analysis</span>
        </div>

        <div className="flex items-center gap-2">
          <span className="text-xs font-mono text-[#475569]">{fmtBytes(file_size)}</span>
      ...
      {/* Chart */}
      <div className="overflow-hidden rounded-lg border border-[#1E293B]" style={{ background: '#020617' }}>
        <svg
        ...
        >
          ...
          {REF.map(r => (
            <g key={r.e}>
              ...
              <text x={PAD_L - 3} y={toY(r.e) + 3.5}
                fontSize={8} fontFamily="monospace" fill="#475569" textAnchor="end">
                {r.label}
              </text>
            </g>
          ))}
          ...
          <text x={PAD_L - 3} y={toY(8) + 3.5} fontSize={8} fontFamily="monospace" fill="#334155" textAnchor="end">8</text>
          <text x={PAD_L - 3} y={toY(0) + 3.5} fontSize={8} fontFamily="monospace" fill="#334155" textAnchor="end">0</text>
          ...
          <text x={PAD_L} y={PAD_T + CHART_H + 13} fontSize={7.5} fontFamily="monospace" fill="#334155">0x0</text>
          <text x={VW - PAD_R} y={PAD_T + CHART_H + 13} fontSize={7.5} fontFamily="monospace" fill="#334155" textAnchor="end">
      ...
      <div className="grid grid-cols-3 gap-3 mt-4">
        ...
        <StatCard
          ...
          sub={CLS_HINT[classification]}
          color={meta.color}
        />
      </div>

      {/* Legend */}
      <div className="flex items-center gap-5 mt-4 pt-3 border-t border-[#1E293B]">
        {([
...
function StatCard({ label, value, sub, color }: { label: string; value: string; sub: string; color: string }) {
  return (
    <div className="p-3 rounded-lg border border-[#1E293B] bg-[#0B1120]">
      <p className="text-[9px] font-mono text-[#475569] uppercase tracking-wider mb-1">{label}</p>
      <p className="text-sm font-mono font-bold" style={{ color }}>{value}</p>
      <p className="text-[9px] font-mono text-[#334155] mt-0.5">{sub}</p>
    </div>
  )
}
